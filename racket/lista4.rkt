...
(displayln "\nMIRROR")
(mirror '(node 4 (node 3 leaf (node 4 (node 3 leaf leaf) (node 42 leaf leaf)))
                 (node 42 (node 54 (node 3 leaf leaf) leaf) leaf)))
...
(define (i-flatten t)
  (define (iter t acc)
    (if (leaf? t)
        acc
        (iter (caddr t) (cons (cadr t) (iter (cadddr t) acc)))))
  (iter t null))

(displayln "\nI-FLATTEN")
(i-flatten '(node 3 (node 4 (node 5 leaf leaf) (node 90 leaf leaf)) (node 7 (node 42 leaf leaf) (node 89 leaf leaf))))

(define (r-flatten t)
  (if [leaf? t]
      null
      (append (r-flatten (node-left t))
              (list (node-val t))
              (r-flatten (node-right t)))))
...
(define (treesort l)
  (define (iter l acc)
    (if (null? l)
        (flatten acc)
        (iter (cdr l) (bst-insert (car l) acc))))
  (iter l 'leaf))

(displayln "\nTREESORT")
(treesort '(1 2 56 8 0 7 42))